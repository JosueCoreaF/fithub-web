...
create table if not exists public.membresias (
	...
	id_cliente uuid not null references public.clientes(id_persona) on delete cascade,
	...
	created_at timestamptz not null default now(),
	updated_at timestamptz not null default now(),
	constraint membresias_estado_check check (estado in ('activa', 'vencida', 'cancelada')),
	constraint membresias_fechas_check check (fecha_vencimiento >= fecha_inicio),
	constraint membresias_costo_check check (costo >= 0)
);
...
create table if not exists public.pagos (
	...
	id_membresia uuid references public.membresias(id_membresia) on delete set null,
	created_at timestamptz not null default now(),
	updated_at timestamptz not null default now(),
	...
	constraint pagos_origen_check check (id_reserva is not null or id_membresia is not null)
);
...
create index if not exists idx_membresias_cliente on public.membresias (id_cliente, fecha_vencimiento desc);
...
create index if not exists idx_pagos_membresia on public.pagos (id_membresia);
...
drop trigger if exists membresias_set_updated_at on public.membresias;
